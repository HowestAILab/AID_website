<template>
    <div class="flex-1 bg-white flex flex-col min-h-0">
      ...
      <div v-else class="flex-1 p-4 overflow-y-auto min-h-0">
        ...
        <div class="grid gap-4 grid-cols-1 sm:grid-cols-[repeat(auto-fit,minmax(280px,1fr))] md:grid-cols-[repeat(auto-fit,minmax(300px,1fr))] lg:grid-cols-[repeat(auto-fit,minmax(320px,1fr))]">
          <UnifiedExerciseCard
            v-for="(exercise, index) in chronologicallyOrderedPins"
            :key="exercise.originalIndex"
            :title="exercise.name"
