...
const beforeCompleted = computed(() => hasBeforeEthics.value && isCompleted(props.exercise.name, 'before'));
const afterCompleted = computed(() => hasAfterEthics.value && isCompleted(props.exercise.name, 'after'));
...
const status = computed(() => {
  ...
  const totalRequired = (hasBeforeEthics.value ? 1 : 0) + (hasAfterEthics.value ? 1 : 0);
  const totalCompleted = (beforeCompleted.value ? 1 : 0) + (afterCompleted.value ? 1 : 0);
...
const getStepClasses = (step: 'before' | 'after') => {
  const isRequired = step === 'before' ? hasBeforeEthics.value : hasAfterEthics.value;
  const isCompleted = step === 'before' ? beforeCompleted.value : afterCompleted.value;
